...
read.sivep <- function(dir, # diretorio onde esta o dado
    ...
    columns <- c("DT_NOTIFIC", "DT_SIN_PRI", "DT_PCR", "DT_DIGITA", "SG_UF",
                 "DT_EVOLUCA", "DT_ENCERRA", "PCR_SARS2", "CLASSI_FIN",
                 "EVOLUCAO", "CO_MUN_RES", "CO_MUN_NOT", "SG_UF_NOT")
    columns <- c(columns, extra_cols)
    col_list <- as.list(rep("c", length(columns)))
    names(col_list) <- columns
